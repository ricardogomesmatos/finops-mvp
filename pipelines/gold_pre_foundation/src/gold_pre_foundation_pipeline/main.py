"""Entry point mínimo da camada Gold Pre-Foundation.

Não replica o orquestrador de 5 camadas do `gcp_labels/main.py` (legado) — fora
de escopo desta entrega, que migra apenas a camada Gold Pre-Foundation. Pensado
para ser invocado por um Cloud Function/Cloud Run Job dedicado a esta camada,
recebendo `invoice_month` via payload da requisição/evento (mesmo padrão de
`pipelines/silver/src/silver_pipeline/main.py`).
"""
...
logger = build_logger(name="gold_pre_foundation_pipeline.main")


def run(invoice_month: str | None = None, bypass_validation: bool = False) -> dict[str, str]:
    """Executa a camada Gold Pre-Foundation para o `invoice_month` informado.
    ...
    logger.info(f"[gold-pre-foundation] Starting gold pre-foundation layer for invoice_month={invoice_month}")
    service = GoldPreFoundationService(bypass_validation=bypass_validation)
    result = service.load_gold_pre_foundation_data(invoice_month)
    logger.info(f"[gold-pre-foundation] Finished with status={result['status']}")
    return result
